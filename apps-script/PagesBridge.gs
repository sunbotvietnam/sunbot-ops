...
const SUNBOT_PAGES_BRIDGE_VERSION = '2026-09-13-v32-school-profile-master-v2';
function handlePagesBridge_(e){
  ...
  try{
    ...
    else if(mode==='v2Profile') result=apiSessionV2ProfileV2(token,String(p.subaction||''),payload);
    else if(mode==='profileBootstrapOnce') result=v2p2BootstrapOnce_(String(p.bootstrap_key||''));
    ...
    else if(mode==='quotationSalesWorkspace') result=apiSessionQuotationSalesWorkspace(token,String(p.subaction||''),payload);
