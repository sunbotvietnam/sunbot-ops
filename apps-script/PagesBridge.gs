...
function handlePagesBridge_(e){
  const p=e&&e.parameter?e.parameter:{};const requestId=String(p.request_id||'').replace(/[^a-zA-Z0-9_-]/g,'').slice(0,80);const mode=String(p.mode||'').trim();const token=String(p.token||'').trim();let payload={};
...
function pagesPinLogin_(email,pin){const normalized=normalizeEmail_(email);const person=findOne_(APP.SHEETS.PEOPLE,'email',normalized);if(!person||!isActiveStatus_(person.trang_thai))throw new Error('Email hoặc mã PIN không đúng.');const value=String(pin||'').trim();const isCeo=String(person.user_id)==='USR-TUONGVAN1906';if(isCeo&&!/^\d{6}$/.test(value))throw new Error('PIN quản trị phải gồm 6 chữ số.');if(!isCeo&&!/^\d{4}$/.test(value))throw new Error('Mã PIN phải gồm 4 chữ số.');if(isCeo)return loginAdminPassword(PASSWORD_AUTH.ADMIN_USERNAME,value);return loginPassword_(String(person.user_id),value);}
function pagesBridgeHtml_(requestId,result,error){const message={type:'sunbot-pages-response',requestId:requestId,ok:!error,result:result||null,error:error||''};const json=JSON.stringify(message).replace(/<\//g,'<\\/');const html='<!doctype html><html><head><meta charset="utf-8"></head><body><script>(function(){try{window.top.postMessage('+json+', '+JSON.stringify(SUNBOT_PAGES_ORIGIN)+');}catch(e){window.parent.postMessage('+json+', "*");}})();<\/script></body></html>';return HtmlService.createHtmlOutput(html).setTitle('SUNBOT OPS Bridge').setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);}
