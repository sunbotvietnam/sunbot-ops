(function(){
  ...
  function applyFastState(data,cache=true){state.boot=data.boot||state.boot;state.summary=data.summary||{};state.dashboard=data.dashboard||state.dashboard||{};state.rows=data.rows||[];state.generatedAt=data.generated_at||state.generatedAt||'';state.shellOnly=!!data.shell_only;if(cache&&!data.shell_only)saveCachedState(data);}

  async function seedIfNeeded(data){if(!data||!data.needs_seed)return;const can=data.boot&&data.boot.can||state.boot?.can||{};const canSync=!!(can['ceo.view']||can['admin.people']||can['account.view_all']);if(!canSync)return;toast('Đang chuẩn bị danh sách trường lần đầu…');try{await longBridge('syncSafe','sync',{},state.token,120000);const fresh=await bridge('fast','load',{force:true},state.token);applyFastState(fresh);renderContent();toast('Đã chuẩn bị xong danh sách trường.');}catch(err){toast(err.message,true);}}

  async function loadWorkspaceAfterLogin_(hasCache){
    if(!hasCache){
      try{const shell=await bridge('fastShell','load',{},state.token);applyFastState(shell,false);state.tasks=[];state.tasksLoaded=false;if(state.tab==='outreach')renderOutreach();if(shell.needs_seed)seedIfNeeded(shell);}catch(e){/* full load below remains fallback */}
    }
    try{const data=await bridge('fast','load',{},state.token);applyFastState(data,true);state.tasks=[];state.tasksLoaded=false;if(state.tab==='outreach')renderOutreach();seedIfNeeded(data);}catch(e){if(!(state.rows||[]).length){const c=document.getElementById('content');if(c)c.innerHTML='<div class="empty"><b>Đăng nhập thành công.</b><br>Chưa tải được danh sách trường. Bấm ↻ để thử lại mà không cần đăng nhập lại.</div>';}toast('Chưa đồng bộ được dữ liệu mới: '+e.message,true);}
  }
  ...
  window.loadApp=async function(){
    try{
      ...
      const restored=restoreCachedState();if(!restored){state.summary={};state.dashboard={};state.rows=[];state.generatedAt='';state.shellOnly=false;}
      renderApp();
      toast(restored?'Đã mở dữ liệu gần nhất. Đang đồng bộ…':'Đăng nhập thành công. Đang tải trường…');
      loadWorkspaceAfterLogin_(restored);
  ...
  window.refreshOutreach=async function(force){try{const data=await bridge('fast','load',{force:!!force},state.token);applyFastState(data,true);if(state.tab==='outreach')renderOutreach();if(data.needs_seed)seedIfNeeded(data);}catch(e){toast(e.message,true);}};
