(function(){
  window.loginView=function(){
    ...
    el('app').innerHTML=`<main class="login-shell"><section class="login-card"><div class="logo">S</div><h1>SUNBOT OPS</h1><p>Vận hành đội ngũ · Tiếp cận trường · Theo dõi cơ hội</p>
      ...
      <input id="loginPin" class="input otp" type="password" inputmode="numeric" maxlength="4" pattern="[0-9]{4}" placeholder="••••" autocomplete="current-password">
      <button class="btn" id="pinLoginBtn">Đăng nhập</button>
      <small>Dùng email đã được cấp quyền trong SUNBOT OPS và mã PIN 4 số của bạn.</small>
  ...
  window.loginWithPin=async function(){
    ...
    if(!/^\d{4}$/.test(pin))return toast('Mã PIN phải gồm 4 chữ số.',true);
