(function(){
  window.loginView=function(){
    ...
    el('app').innerHTML=`<main class="login-shell"><section class="login-card"><img class="login-logo" src="https://sunbotvietnam.github.io/portal/profile/assets/images/logo-sunbot.png" alt="Sunbot"><h1>SUNBOT OPS</h1><p>Phát triển trường · Cơ hội · Việc tiếp theo</p>
      ...
      <input id="loginEmail" class="input" type="text" value="${identifier}" placeholder="Ví dụ: admin, staff1" autocomplete="username">
      ...
      <small>ID và mật khẩu được Admin cấp trực tiếp trong SUNBOT OPS. Hệ thống không gửi mã qua email.</small>
  ...
  window.loginWithPin=async function(){
    const identifier=String(el('loginEmail')?.value||'').trim();const pin=String(el('loginPin')?.value||'').trim();
    if(identifier.length<3)return toast('Hãy nhập ID đăng nhập.',true);if(!/^\d{6}$/.test(pin))return toast('Mật khẩu phải gồm 6 số.',true);
