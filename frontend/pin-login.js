(function(){
  window.loginView=function(){
    ...
    el('app').innerHTML=`<main class="login-shell"><section class="login-card"><div class="logo">S</div><h1>SUNBOT OPS</h1><p>Tiếp cận trường · Công việc · Theo dõi cơ hội</p>
      ...
      <small>Chỉ tài khoản đã được cấp quyền trong SUNBOT OPS mới đăng nhập được.</small>
  ...
  window.loginWithPin=async function(){
    ...
    if(!/^\d{6}$/.test(pin))return toast('Mã PIN phải gồm 6 số.',true);
    state.email=email;sessionStorage.setItem(EMAIL_KEY,email);
    el('loginMsg').textContent='Đang đăng nhập...';
    try{
      const r=await call('pinLogin','',{email,pin},'');
      state.token=r.token;sessionStorage.setItem(SESSION_KEY,state.token);
      el('loginPin').value='';
      await loadApp();
    }catch(e){
