...
for (const token of ['function apiSessionCommercial(','commercialCreateMarketSignal_','commercialCreateOpportunity_','commercialReviewMarketSignal_','commercialKpi_','THI_TRUONG_TIN_HIEU','CHAO_BAN_THI_TRUONG']) {
  if (!commercial.includes(token)) { console.error('Commercial backend thiếu chức năng:', token); failed = true; }
}
...
for (const token of ['apiSessionCommercial','Ghi nhận thị trường','Cơ hội','KPI','saveMarketSignal','saveOpportunity']) {
  if (!commercialUi.includes(token)) { console.error('Commercial UI thiếu chức năng:', token); failed = true; }
}
const index = fs.readFileSync('apps-script/Index.html','utf8');
for (const token of ['screen-commercial','CommercialUi','Thị trường & Cơ hội']) {
  if (!index.includes(token)) { console.error('Index chưa nối Commercial UI:', token); failed = true; }
}
